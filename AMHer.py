...
in_file = open('C:\\Users\\falli\\PycharmProjects\\Get_Data_AAVSO\\AMHer_Vis.txt','w+')      #初始化用于数据保存的文件
urlmode='https://www.aavso.org/apps/webobs/results/?star=AM+HER&num_results=200&page='       #url模板
for x in range(1,312):
    html = getHtml(urlmode+str(x))
    getdata(html)
    print(urlmode+str(x))                                                 #输出请求的url
in_file.close()                                                           #关闭数据文件
